...
#SBATCH -o /sciclone/home/mrwood01/BIOCOMPUTING/lessons/lesson_08/logs/coverage_%j.out
#SBATCH -e /sciclone/home/mrwood01/BIOCOMPUTING/lessons/lesson_08/logs/coverage_%j.err
...
do
    ...
    if [[ ! -f "${contigs}.1.bt2" ]]; then
        echo "  [index] bowtie2-build ${contigs}"
    ...
    if [[ ! -s "${bam}" ]]; then
        ...
        bowtie2 -x "${contigs}" -1 "$fwd" -2 "$rev" -p 8 \
